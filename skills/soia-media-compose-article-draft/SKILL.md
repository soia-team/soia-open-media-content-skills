...
媒体内容工作流的**成文环节（写）**：把 `distill` 产出的观点，写成一篇可发布的文章草稿。
...
```text
~/.config/soia-skills/soia-media-compose-article-draft/config.yml
SOIA_MEDIA_COMPOSE_ARTICLE_DRAFT_CONFIG_FILE=<custom-config-path>
```
...
2. **沿用角度与风格**：先用本次已给定的角度、大纲、标题和平台要求；缺少定位时按需读 vault 的个人说明书。只有关键差异会改变结果时询问，不重复确认已给定内容。
3. **搭结构**：开头（钩子/问题）→ 主体（你的观点 + 论据）→ 结尾（升华/行动）。用户要求先审大纲时等待确认；否则沿已给定或合理推导的大纲直接成文，并允许后续修改。
...
6. **回执**：告知草稿路径、字数、结构，提示"要改哪段告诉我；满意后按目标平台选择 `publish-*` 家族"。

## 标题工序

沿用用户已给定的标题；未指定时拟一个与正文一致的工作标题并落草稿。用户要求标题选择时再提供候选；标题定稿不构成发布授权。
