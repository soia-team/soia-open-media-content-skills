#!/usr/bin/env python3
"""Host-agnostic X Articles draft filler for soia-media-publish-x-article.
...
soia-media-publish-x-thread's scripts/x_post.py.
...
~/.config/soia-skills/soia-media-publish-x-thread/x-profile
...
— that is the shared media profile. If only the legacy PKM profile exists,
the resolver reuses it so migration does not force a redundant login. Do not
"fix" this by pointing it at a new soia-media-publish-x-article directory;
that would defeat the sharing.
...
def screenshot_path(prefix: str) -> Path:
    d = Path(tempfile.gettempdir()) / "soia-media-publish-x-article"
