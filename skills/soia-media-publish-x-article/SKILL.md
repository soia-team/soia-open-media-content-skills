...
```bash
npx skills add soia-team/soia-open-media-content-skills -g -a '*' -s soia-media-publish-x-article -y
```
...
- 跨平台 Python 3.9+（macOS / Windows / Linux 均可）：主路线用 Playwright 自带浏览器 + 原生剪贴板 API，不依赖 osascript / sips 等系统专属工具。
  ...
  ```
- 登录态与 `soia-media-publish-x-thread` 共用同一份本地 profile 目录（`~/.config/soia-skills/soia-media-publish-x-thread/x-profile`，可用 `--profile-dir` / `SOIA_X_PROFILE_DIR` 覆盖；脚本会对旧命名空间做只读回退）：已经用 `x_post.py login` 登录过就直接可用；未登录时去该技能跑一次 `python3 scripts/x_post.py login`（本技能自身不提供 `login` 子命令，靠共用 profile 免二次登录）。
...
| 「发条 X/发个推」（普通短帖） | **不归本技能**：短帖/thread 文本用 `soia-media-publish-x-thread`；本技能只做 Articles 长文草稿 |
...
- **登录**：本技能自身不提供 `login` 子命令。未登录时去 `soia-media-publish-x-thread` 跑一次 `python3 scripts/x_post.py login`，两边共用同一份本地 profile，登录一次两边都能用。
- `check` 返回四种状态之一：
