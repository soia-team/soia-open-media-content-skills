#!/usr/bin/env python3
"""Host-agnostic X post/thread filler for soia-media-publish-x-thread.
...
~/.config/soia-skills/soia-media-publish-x-thread/x-profile. If only the
legacy profile exists, it is reused so migration does not force a new login.
