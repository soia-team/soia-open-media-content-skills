...
## imagegen prompt 写法（实测教训）

- **排版说明与逐字文字必须可区分**：颜色、字号、位置等说明一律放括号内，并在 prompt 里显式声明「括号内是排版说明，不得出现在画面里」。不要用「绿色：xxx」这种行首标注体——实测（2026-08-07）模型把行首的『绿色：』『灰蓝：』当正文画进了图，返工一张。
- 逐字文字用「必须逐字出现（一个字不能错）」的独立小节列出；禁项单列（二维码、水印、未列出的文字、繁体）。
- 每张 prompt 末尾要求执行侧「生成后用 shell 确认文件存在于目标路径并报告实际像素尺寸」。
