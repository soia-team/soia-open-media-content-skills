...
```bash
claude plugin update soia-media-content@soia   # 交付走插件市场，勿装全局
```

Forbidden outside local testing:

```bash
npx skills add /absolute/local/path/to/soia-open-media-content-skills -g -a '*' -s <skill-name> -y
```
...
```bash
cd <your-local-checkout>/soia-open-media-content-skills
...
3. **Users update** with `claude plugin update soia-media-content@soia` or
...
claude plugin update soia-media-content@soia
```
